...
import com.theironingboard.material.R;
import com.theironingboard.material.drawable.DividerDrawable;
import com.theironingboard.material.util.ThemeUtil;
import com.theironingboard.material.util.ViewUtil;
...
@SuppressLint("NewApi")
public class EditTextNoDivider extends FrameLayout {
	...
	private ColorStateList mDividerColors, mSelected;
	...
	public EditTextNoDivider(Context context) {
		super(context);

		init(context, null, 0, 0);
	}

	public EditTextNoDivider(Context context, AttributeSet attrs) {
		super(context, attrs);

		init(context, attrs, 0, 0);
	}

	public EditTextNoDivider(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void applyStyle(Context context, AttributeSet attrs,
		...
		mSupportMode = a.getInteger(R.styleable.EditText_et_supportMode,
				SUPPORT_MODE_NONE);
		mAutoCompleteMode = a.getInteger(
				R.styleable.EditText_et_autoCompleteMode,
				AUTOCOMPLETE_MODE_NONE);

		switch (mAutoCompleteMode) {
		case AUTOCOMPLETE_MODE_SINGLE:
			mInputView = new InternalAutoCompleteTextView(context, attrs,
					defStyleAttr);
			break;
		case AUTOCOMPLETE_MODE_MULTI:
			mInputView = new InternalMultiAutoCompleteTextView(context, attrs,
					defStyleAttr);
			break;
		default:
		...
		mDividerColors = a
				.getColorStateList(R.styleable.EditText_et_dividerColor);
		mDividerErrorColors = a
				.getColorStateList(R.styleable.EditText_et_dividerErrorColor);
		mSelected = a.getColorStateList(R.styleable.EditText_et_dividerSelect);
		...
		if (mDividerErrorColors == null)
			mDividerErrorColors = ColorStateList.valueOf(ThemeUtil.colorAccent(
					context, 0xFFFF0000));

		int dividerHeight = a.getDimensionPixelOffset(
				R.styleable.EditText_et_dividerHeight, 0);
		int dividerPadding = a.getDimensionPixelOffset(
				R.styleable.EditText_et_dividerPadding, 0);
		int dividerAnimDuration = a.getInteger(
				R.styleable.EditText_et_dividerAnimDuration,
				context.getResources().getInteger(
						android.R.integer.config_shortAnimTime));
		mDividerCompoundPadding = a.getBoolean(
				R.styleable.EditText_et_dividerCompoundPadding, true);
		mInputView.setPadding(0, 0, 0, dividerPadding + dividerHeight);

		mDivider = new DividerDrawable(
				dividerHeight,
				mDividerCompoundPadding ? mInputView.getTotalPaddingLeft() : 0,
				mDividerCompoundPadding ? mInputView.getTotalPaddingRight() : 0,
				mDividerColors, dividerAnimDuration);
...
//			mInputView.setBackgroundDrawable(mDivider);
		...
		addView(mInputView, new ViewGroup.LayoutParams(
		...
		if (mLabelEnable) {
			...
			int labelPadding = a.getDimensionPixelOffset(
					R.styleable.EditText_et_labelPadding, 0);
			int labelTextSize = a.getDimensionPixelSize(
					R.styleable.EditText_et_labelTextSize, 0);
			ColorStateList labelTextColor = a
					.getColorStateList(R.styleable.EditText_et_labelTextColor);
			int labelTextAppearance = a.getResourceId(
					R.styleable.EditText_et_labelTextAppearance, 0);
			int labelEllipsize = a.getInteger(
					R.styleable.EditText_et_labelEllipsize, 0);
			mLabelInAnimId = a.getResourceId(
					R.styleable.EditText_et_labelInAnim, 0);
			mLabelOutAnimId = a.getResourceId(
					R.styleable.EditText_et_labelOutAnim, 0);

			mLabelView.setPadding(mDivider.getPaddingLeft(), 0,
					mDivider.getPaddingRight(), labelPadding);
			if (labelTextAppearance > 0)
				mLabelView.setTextAppearance(context, labelTextAppearance);
			if (labelTextSize > 0)
				mLabelView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
						labelTextSize);
			...
			addView(mLabelView, 0, new ViewGroup.LayoutParams(
					ViewGroup.LayoutParams.MATCH_PARENT,
					ViewGroup.LayoutParams.WRAP_CONTENT));
		}
		...
		if (mSupportMode != SUPPORT_MODE_NONE) {
			...
			int supportPadding = a.getDimensionPixelOffset(
					R.styleable.EditText_et_supportPadding, 0);
			int supportTextSize = a.getDimensionPixelSize(
					R.styleable.EditText_et_supportTextSize, 0);
			mSupportColors = a
					.getColorStateList(R.styleable.EditText_et_supportTextColor);
			mSupportErrorColors = a
					.getColorStateList(R.styleable.EditText_et_supportTextErrorColor);
			int supportTextAppearance = a.getResourceId(
					R.styleable.EditText_et_supportTextAppearance, 0);
			int supportEllipsize = a.getInteger(
					R.styleable.EditText_et_supportEllipsize, 0);
			int supportMaxLines = a.getInteger(
					R.styleable.EditText_et_supportMaxLines, 0);
			int supportLines = a.getInteger(
					R.styleable.EditText_et_supportLines, 0);
			boolean supportSingleLine = a.getBoolean(
					R.styleable.EditText_et_supportSingleLine, false);

			mSupportView.setPadding(mDivider.getPaddingLeft(), supportPadding,
					mDivider.getPaddingRight(), 0);
			mSupportView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
					supportTextSize);
			...
			case SUPPORT_MODE_CHAR_COUNTER:
				mSupportMaxChars = a.getInteger(
						R.styleable.EditText_et_supportMaxChars, 0);
			...
			case SUPPORT_MODE_HELPER_WITH_ERROR:
				...
				mSupportHelper = ThemeUtil.getString(a,
						R.styleable.EditText_et_helper, supportHelper);
				setError(ThemeUtil.getString(a, R.styleable.EditText_et_error,
						supportError));
				break;
			}
			addView(mSupportView, new ViewGroup.LayoutParams(
					ViewGroup.LayoutParams.MATCH_PARENT,
					ViewGroup.LayoutParams.WRAP_CONTENT));
		}
		...
		if (mLabelEnable) {
			...
			mLabelView.setVisibility(TextUtils.isEmpty(mInputView.getText()
					.toString()) ? View.INVISIBLE : View.VISIBLE);
		}
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int tempWidthSpec = widthMode == MeasureSpec.UNSPECIFIED ? widthMeasureSpec
				: MeasureSpec.makeMeasureSpec(widthSize - getPaddingLeft()
						- getPaddingRight(), widthMode);
		int tempHeightSpec = MeasureSpec.makeMeasureSpec(0,
		...
		case MeasureSpec.UNSPECIFIED:
			width = Math.max(labelWidth, Math.max(inputWidth, supportWidth))
					+ getPaddingLeft() + getPaddingRight();
			break;
		case MeasureSpec.AT_MOST:
			width = Math.min(widthSize,
					Math.max(labelWidth, Math.max(inputWidth, supportWidth))
							+ getPaddingLeft() + getPaddingRight());
			break;
		...
		switch (heightMode) {
		case MeasureSpec.UNSPECIFIED:
			height = labelHeight + inputHeight + supportHeight
					+ getPaddingTop() + getPaddingBottom();
			break;
		case MeasureSpec.AT_MOST:
			height = Math.min(heightSize, labelHeight + inputHeight
					+ supportHeight + getPaddingTop() + getPaddingBottom());
			break;
		case MeasureSpec.EXACTLY:
			height = heightSize;
			break;
		...
		tempWidthSpec = MeasureSpec.makeMeasureSpec(width - getPaddingLeft()
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mLabelView != null) {
			mLabelView.layout(childLeft, childTop, childRight, childTop
					+ mLabelView.getMeasuredHeight());
			childTop += mLabelView.getMeasuredHeight();
		}

		if (mSupportView != null) {
			mSupportView.layout(childLeft,
					childBottom - mSupportView.getMeasuredHeight(), childRight,
					childBottom);
	...
	public void setError(CharSequence error) {
		...
		if (mSupportMode != SUPPORT_MODE_HELPER
				&& mSupportMode != SUPPORT_MODE_HELPER_WITH_ERROR)
			return;
		...
		if (mSupportError != null) {
			...
			mSupportView
					.setText(mSupportMode == SUPPORT_MODE_HELPER ? mSupportError
							: TextUtils.concat(mSupportHelper, ", ",
									mSupportError));
		} else {
			mSupportView.setTextColor(mSupportColors);
	...
	private void updateCharCounter(int count) {
		if (count == 0) {
			...
			if (mSupportMaxChars > 0) {
				mSupportView
						.setTextColor(count > mSupportMaxChars ? mSupportErrorColors
								: mSupportColors);
				mDivider.setColor(count > mSupportMaxChars ? mDividerErrorColors
						: mDividerColors);
	...
	protected CharSequence convertSelectionToString(Object selectedItem) {
		switch (mAutoCompleteMode) {
		case AUTOCOMPLETE_MODE_SINGLE:
			return ((InternalAutoCompleteTextView) mInputView)
					.superConvertSelectionToString(selectedItem);
		case AUTOCOMPLETE_MODE_MULTI:
			return ((InternalMultiAutoCompleteTextView) mInputView)
					.superConvertSelectionToString(selectedItem);
		default:
			return null;
		}
	...
	protected void performFiltering(CharSequence text, int keyCode) {
		switch (mAutoCompleteMode) {
		case AUTOCOMPLETE_MODE_SINGLE:
			((InternalAutoCompleteTextView) mInputView).superPerformFiltering(
					text, keyCode);
			break;
		case AUTOCOMPLETE_MODE_MULTI:
			((InternalMultiAutoCompleteTextView) mInputView)
					.superPerformFiltering(text, keyCode);
			break;
		}
	}
	 ...
	 * <p>
	 * Starts filtering the content of the drop down list. The filtering pattern
	 * is the specified range of text from the edit box. Subclasses may override
	 * this method to filter with a different pattern, for instance a smaller
	...
	protected void performFiltering(CharSequence text, int start, int end,
		...
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_MULTI)
			((InternalMultiAutoCompleteTextView) mInputView)
					.superPerformFiltering(text, start, end, keyCode);
	}
	...
	public int getDropDownHorizontalOffset() {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			return 0;
		return ((AutoCompleteTextView) mInputView)
	...
	public void onFilterComplete(int count) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_SINGLE)
			((InternalAutoCompleteTextView) mInputView)
					.superOnFilterComplete(count);
	...
	/**
	 * Convenience for
	 * {@link android.text.Selection#setSelection(Spannable, int)}.
	 */
	...
	/**
	 * Convenience for
	 * {@link android.text.Selection#setSelection(Spannable, int, int)}
	 * .
	 */
	 ...
	 * erroneously not called after {@link #setText} calls. Now, doing
	 * {@link #setText} if there are any text changed listeners forces the
	 * buffer type to Editable if it would not otherwise be and does call this
	 * method.
	 */
	...
	public boolean getIncludeFontPadding() {
		return Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN
				&& mInputView.getIncludeFontPadding();
	}
	...
	public float getLetterSpacing() {
		return Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? mInputView
				.getLetterSpacing() : 0;
	}
	 ...
	 * {@link android.text.method.LinkMovementMethod} if
	 * {@link #setAutoLinkMask} has been set to nonzero and links are detected
	 * in {@link #setText}. The default is true.
	 *
	 ...
	 *
	 * @return the default {@link Locale} of the text in this
	 *         TextView.
	 */
	@TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1)
	...
	public int getTotalPaddingBottom() {
		return getPaddingBottom()
				+ mInputView.getTotalPaddingBottom()
				+ (mSupportMode != SUPPORT_MODE_NONE ? mSupportView.getHeight()
						: 0);
	}
	...
	public int getTotalPaddingTop() {
		return getPaddingTop() + mInputView.getTotalPaddingTop()
				+ (mLabelEnable ? mLabelView.getHeight() : 0);
	}
	 ...
	 * {@link URLSpan#getURL} on them to find where they link
	 * to or use {@link android.text.Spanned#getSpanStart} and
	 * {@link android.text.Spanned#getSpanEnd} to find the region of the text
	 ...
	 * types. This is done by adding {@link android.text.style.SuggestionSpan}s
	 * to the text.
	 *
	...
	public boolean isSuggestionsEnabled() {
		return Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH
				&& mInputView.isSuggestionsEnabled();
	}
	 ...
	 * TextView, the content of an {@link EditTextNoDivider} can always be selected,
	...
	/**
	 * Called by the framework in response to a text completion from the current
	 * input method, provided by it calling
	...
	/**
	 * Called by the framework in response to a text auto-correction (such as
	 * fixing a typo using a a dictionnary) from the current input method,
	 * provided by it calling
	 * {@link InputConnection#commitCorrection}
	 * InputConnection.commitCorrection()}. The default implementation flashes
	 * the background of the corrected word to provide feedback to the user.
	 *
	 * @param info
	...
	public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			return ((InternalEditText) mInputView)
					.superOnCreateInputConnection(outAttrs);
		else if (mAutoCompleteMode == AUTOCOMPLETE_MODE_SINGLE)
			return ((InternalAutoCompleteTextView) mInputView)
					.superOnCreateInputConnection(outAttrs);
		else
	 ...
	 * {@link #setOnEditorActionListener}, or perform a standard operation for
	 * {@link EditorInfo#IME_ACTION_NEXT
	 * EditorInfo.IME_ACTION_NEXT},
	 * {@link EditorInfo#IME_ACTION_PREVIOUS
	 * EditorInfo.IME_ACTION_PREVIOUS}, or
	...
	public void onEditorAction(int actionCode) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			((InternalAutoCompleteTextView) mInputView)
					.superOnEditorAction(actionCode);
		else
			((InternalMultiAutoCompleteTextView) mInputView)
					.superOnEditorAction(actionCode);
	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			return ((InternalEditText) mInputView).superOnKeyDown(keyCode,
					event);
		else if (mAutoCompleteMode == AUTOCOMPLETE_MODE_SINGLE)
			return ((InternalAutoCompleteTextView) mInputView).superOnKeyDown(
					keyCode, event);
		else
			return ((InternalMultiAutoCompleteTextView) mInputView)
	...
	public boolean onKeyMultiple(int keyCode, int repeatCount, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			return ((InternalEditText) mInputView).superOnKeyMultiple(keyCode,
					repeatCount, event);
		else if (mAutoCompleteMode == AUTOCOMPLETE_MODE_SINGLE)
			return ((InternalAutoCompleteTextView) mInputView)
					.superOnKeyMultiple(keyCode, repeatCount, event);
		else
			return ((InternalMultiAutoCompleteTextView) mInputView)
					.superOnKeyMultiple(keyCode, repeatCount, event);
	}
	...
	public boolean onKeyPreIme(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			return ((InternalEditText) mInputView).superOnKeyPreIme(keyCode,
					event);
		else if (mAutoCompleteMode == AUTOCOMPLETE_MODE_SINGLE)
			return ((InternalAutoCompleteTextView) mInputView)
					.superOnKeyPreIme(keyCode, event);
		else
			return ((InternalMultiAutoCompleteTextView) mInputView)
					.superOnKeyPreIme(keyCode, event);
	}
	...
	public boolean onKeyShortcut(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			return ((InternalEditText) mInputView).superOnKeyShortcut(keyCode,
					event);
		else if (mAutoCompleteMode == AUTOCOMPLETE_MODE_SINGLE)
			return ((InternalAutoCompleteTextView) mInputView)
					.superOnKeyShortcut(keyCode, event);
		else
			return ((InternalMultiAutoCompleteTextView) mInputView)
					.superOnKeyShortcut(keyCode, event);
	}
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			...
			return ((InternalAutoCompleteTextView) mInputView).superOnKeyUp(
					keyCode, event);
		else
			return ((InternalMultiAutoCompleteTextView) mInputView)
					.superOnKeyUp(keyCode, event);
	}

	public void setOnSelectionChangedListener(
			TextView.OnSelectionChangedListener listener) {
	...
	protected void onSelectionChanged(int selStart, int selEnd) {
		...
		if (mAutoCompleteMode == AUTOCOMPLETE_MODE_NONE)
			((InternalEditText) mInputView).superOnSelectionChanged(selStart,
					selEnd);
		else if (mAutoCompleteMode == AUTOCOMPLETE_MODE_SINGLE)
			((InternalAutoCompleteTextView) mInputView)
					.superOnSelectionChanged(selStart, selEnd);
		else
			((InternalMultiAutoCompleteTextView) mInputView)
					.superOnSelectionChanged(selStart, selEnd);
	...
	public void setCompoundDrawablePadding(int pad) {
		...
		if (mDividerCompoundPadding) {
			mDivider.setPadding(mInputView.getTotalPaddingLeft(),
					mInputView.getTotalPaddingRight());
			if (mLabelEnable)
				mLabelView.setPadding(mDivider.getPaddingLeft(),
						mLabelView.getPaddingTop(), mDivider.getPaddingRight(),
						mLabelView.getPaddingBottom());
			if (mSupportMode != SUPPORT_MODE_NONE)
				mSupportView.setPadding(mDivider.getPaddingLeft(),
						mSupportView.getPaddingTop(),
						mDivider.getPaddingRight(),
						mSupportView.getPaddingBottom());
		}
	}
	 ...
	 * there. The Drawables must already have had
	...
	public void setCompoundDrawablesRelativeWithIntrinsicBounds(Drawable start,
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1)
			mInputView.setCompoundDrawablesRelativeWithIntrinsicBounds(start,
					top, end, bottom);
		else
			mInputView.setCompoundDrawablesWithIntrinsicBounds(start, top, end,
					bottom);
	}
	...
	public void setCompoundDrawablesWithIntrinsicBounds(Drawable left,
		...
		mInputView.setCompoundDrawablesWithIntrinsicBounds(left, top, right,
				bottom);
	}
	 ...
	 * {@link android.view.Menu#removeItem(int)} and passing
	 * {@link android.R.id#selectAll}, {@link android.R.id#cut},
	 * {@link android.R.id#copy} or {@link android.R.id#paste} ids as
	 * parameters.
	 *
	 ...
	 * {@link EditorInfo#actionLabel} and
	 ...
	 * holding an {@link android.R.styleable#InputExtras &lt;input-extras&gt;}
	 * XML tree.
	 *
	 ...
	 * @attr ref android.R.styleable#TextView_editorExtras
	 */
	public void setInputExtras(int xmlResId) throws XmlPullParserException,
			IOException {
	 ...
	 * {@link EditorInfo#inputType}. This will take
	 * care of changing the key listener, by calling
	 * {@link #setKeyListener(KeyListener)}, to match the
	 * given content type. If the given content type is
	 * {@link EditorInfo#TYPE_NULL} then a soft
	 * keyboard will not be displayed for this text view.
	 *
	 * Note that the maximum number of displayed lines (see
	 * {@link #setMaxLines(int)}) will be modified if you change the
	 * {@link EditorInfo#TYPE_TEXT_FLAG_MULTI_LINE}
	 ...
	 * view. Key presses in software input methods will generally not trigger
	 * the methods of this listener.
	 *
	 * @param l
	...
	/**
	 * Set the default {@link Locale} of the text in this TextView to
	 ...
	 * you may need to use {@link #setTypeface(Typeface, int)}
	...
	/**
	 * It would be better to rely on the input type for everything. A password
	 * inputType should have a password transformation. We should hence use
	 * isPasswordInputType instead of this method.
	 *
	 * We should: - Call setInputType in setKeyListener instead of changing the
	 * input type directly (which would install the correct transformation). -
	 * Refuse the installation of a non-password transformation in
	 * setTransformation if the input type is password.
	 *
	 * However, this is like this for legacy reasons and we cannot break
	 * existing apps. This method is useful since it matches what the user can
	 * see (obfuscated text or not).
	 *
	 * @return true if the current transformation method is of the password
	 *         type.
	 */
	...
	public boolean canCut() {
		return !hasPasswordTransformationMethod() && getText().length() > 0
				&& hasSelection() && getKeyListener() != null;
	}
	...
	public boolean canPaste() {
		return (getKeyListener() != null && getSelectionStart() >= 0
				&& getSelectionEnd() >= 0 && ((ClipboardManager) getContext()
	...
	private class InternalEditText extends android.widget.EditText {
		...
		@Override
		public void onCommitCompletion(CompletionInfo text) {
			EditTextNoDivider.this.onCommitCompletion(text);
		}

		@Override
		public void onCommitCorrection(CorrectionInfo info) {
			EditTextNoDivider.this.onCommitCorrection(info);
		}

		@Override
		public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
			return EditTextNoDivider.this.onCreateInputConnection(outAttrs);
		}

		@Override
		public void onEditorAction(int actionCode) {
			EditTextNoDivider.this.onEditorAction(actionCode);
		}

		@Override
		public boolean onKeyDown(int keyCode, KeyEvent event) {
			return EditTextNoDivider.this.onKeyDown(keyCode, event);
		}

		@Override
		public boolean onKeyMultiple(int keyCode, int repeatCount,
				KeyEvent event) {
			return EditTextNoDivider.this.onKeyMultiple(keyCode, repeatCount, event);
		}

		@Override
		public boolean onKeyPreIme(int keyCode, KeyEvent event) {
			return EditTextNoDivider.this.onKeyPreIme(keyCode, event);
		}

		@Override
		public boolean onKeyShortcut(int keyCode, KeyEvent event) {
			return EditTextNoDivider.this.onKeyShortcut(keyCode, event);
		}

		@Override
		public boolean onKeyUp(int keyCode, KeyEvent event) {
			return EditTextNoDivider.this.onKeyUp(keyCode, event);
		}

		@Override
		protected void onSelectionChanged(int selStart, int selEnd) {
			EditTextNoDivider.this.onSelectionChanged(selStart, selEnd);
		}
	...
	private class InternalAutoCompleteTextView extends
		...
		@Override
		protected CharSequence convertSelectionToString(Object selectedItem) {
			return EditTextNoDivider.this.convertSelectionToString(selectedItem);
		}

		@Override
		protected void performFiltering(CharSequence text, int keyCode) {
			EditTextNoDivider.this.performFiltering(text, keyCode);
		}

		@Override
		protected void replaceText(CharSequence text) {
			EditTextNoDivider.this.replaceText(text);
		}

		@Override
		protected Filter getFilter() {
			return EditTextNoDivider.this.getFilter();
		}

		@Override
		public void onFilterComplete(int count) {
			EditTextNoDivider.this.onFilterComplete(count);
		}
	...
	private class InternalMultiAutoCompleteTextView extends
		...
		protected void performFiltering(CharSequence text, int start, int end,
				int keyCode) {
			EditTextNoDivider.this.performFiltering(text, start, end, keyCode);
		}
		...
		void superPerformFiltering(CharSequence text, int start, int end,
				int keyCode) {
